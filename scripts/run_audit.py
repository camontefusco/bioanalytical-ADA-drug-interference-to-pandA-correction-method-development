...
def simulate(seed: int, cfg: dict, n: int | None = None) -> dict:
    ...
    return {
        ...
        "standard_expected_prev": float(standard_probability.mean()),
        "panda_expected_prev": float(panda_probability.mean()),
        "standard_observation_noise_pp": float((standard.mean() - standard_probability.mean()) * 100),
        "panda_observation_noise_pp": float((panda.mean() - panda_probability.mean()) * 100),
